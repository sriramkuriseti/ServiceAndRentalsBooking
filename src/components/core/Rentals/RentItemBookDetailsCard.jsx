...
import "./RentItemBookDetailsCard.css"; // Assuming you have a CSS file for this component

import { setWishlist } from "../../../slices/wishlist";
import { ACCOUNT_TYPE } from "../../../utils/constants";

import { addRentItemToWishlist, removeRentItemFromWishlist,fetchWishList } from "../../../services/operations/wishlistAPI";

function RentItemBookDetailsCard({ handleBookRentItem }) {
  ...
  useEffect(() => {
    console.log("service id :",rentItemId)
    console.log(wishlist)
    console.log(wishlist &&wishlist?.rents?.some((item) => item === rentItemId))
    if (wishlist && wishlist?.rents?.some((item) => item._id === rentItemId)) {
  ...
  const handleHeartClick = async () => {
    ...
    if (token) {
      if (!isHeartFilled) {
        ...
        try {
          setLoading(true);
          const res = await addRentItemToWishlist({ userId: user._id, rentItemId: rent._id }, token);
          setLoading(false);
        ...
        try {
          setLoading(true);
          const res = await removeRentItemFromWishlist({ userId: user._id, rentItemId: rent._id }, token);
          setLoading(false);
          dispatch(setWishlist(res));
      ...
      <div className="px-4">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <button className="yellowButton w-10/12" onClick={handleBookRentItem}>
              Book Now
            </button>
        ...
        <div>
          <p className="my-2 text-xl font-semibold">This Rent Item Includes:</p>
...
export default RentItemBookDetailsCard;
