...
import { fetchRentItemsBookedByUser,cancelRentItemSlot} from "../../../../../services/operations/rentDetailsAPI";
import { setWriteReview,setRent } from "../../../../../slices/rentSlice";
const tabData = [
...
export default function DetailsRentTable({ type }) {
  ...
  const modifiedType = type.slice(0, -1);
   ...
   const fetchData = async () => {
      try {
        ...
        const res = await fetchRentItemsBookedByUser({userId : user._id},token);
        console.log("Before making  fetch History call",data);
        console.log("fetch History responce: ", res);
       // setResponse(res);n
        setData(res)
      } catch (error) {
        console.log("Could not fetch Rent Item Details", error);
      }
    }
  // useEffect(() => {
  //   fetchData();
  ...
  const handleCancel = async (slotId,rentItemId) => {
    setLoading(true);
    if (type === "rents") {
      await cancelRentItemSlot({rentItemId:rentItemId ,slotId:slotId,userId: user._id}, token);
  ...
  const handleWriteReview = async (rent)=>{
    dispatch(setRent(rent));
    dispatch(setWriteReview(true));
    navigate(`/${type}/${rent._id}`)
  }
              ...
              >
               ...
               {/* ,{ state: { isProvider: user.accountType === ACCOUNT_TYPE.PROVIDER } } */}
                ...
                {item?.slot?.progress !== "Completed" ? (
                    ...
                    onClick={() => {
                      setConfirmationModal({
                        ...
                        btn1Handler: !loading
                          ...
                          : () => {},
                        btn2Handler: !loading
                          ? () => handleCancel(item?._id,item?.rent?._id)
                          : () => {},
                      });
                    }}
                    title="Cancel"
                  ...
                  <button
                    disabled={loading}
                    onClick={() => handleWriteReview(item?.rent)}
                    title="Cancel"
                    className="flex flex-row px-1 transition-all duration-200 text-[#e3e65a] hover:scale-110 hover:text-[#ff0000]"
                  >
