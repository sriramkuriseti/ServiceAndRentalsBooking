...
import { addRentItemSlots } from '../../../../../../../services/operations/rentDetailsAPI'
import { setRent, setEditRent, setStep } from '../../../../../../../slices/rentSlice'

import IconBtn from '../../../../../../Common/IconBtn'
import NestedView from './NestedVIew'
import ConfirmationModal from '../../../../../../Common/ConfirmationModal'

export default function RentItemBuilderForm() {
  ...
  const onSubmit = async (data) => {
    ...
    console.log("Data before sending:", { ...data, rentItemId: rent._id }); 

    setLoading(true)


    const result = await addRentItemSlots({ ...data, rentItemId: rent._id }, token)
    if (result) {
  ...
  const goToNext = () => {
    if (!rent.slots || rent.slots.length === 0) {
  ...
  return (
    <div className="space-y-8 rounded-md border-[1px] border-richblack-700 bg-richblack-800 p-6">
      <p className="text-2xl font-semibold text-richblack-5">Rent Item Slot Builder</p>
      ...
      {rent.slots && rent.slots.length > 0 && (
        <NestedView  />
      )}
