...
import ServiceReviewModal from "../components/core/Service/ServiceReviewModel"
import Footer from "../components/Common/Footer";
import ServiceReviewSlider  from "../components/Common/ServiceReviewSlider"
import RatingStars from "../components/Common/RatingStars";
import ServiceBookDetailsCard from "../components/core/Service/ServiceBookDetailsCard";
import { formatDate } from "../services/formatDate";
import { fetchServiceDetails } from "../services/operations/serviceDetailsAPI";
import GetAvgRating from "../utils/avgRating";
import Error from "./Error";
import { setService } from "../slices/serviceSlice"; // Import the action creator

export default function ServiceDetails() {
  const { user } = useSelector((state) => state.profile);
  const { token } = useSelector((state) => state.auth);
  const { loading } = useSelector((state) => state.profile);
  const { service ,enableWriteReview} = useSelector((state) => state.service);
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.log("Before setting Service details in Details Page ", service);
        console.log("service details response from server in Details page: ", res);
       // setResponse(res);
        dispatch(setService(res)); // Correctly dispatch the action
  ...
  console.log("After setting Service details in Details Page ", service);
  
  const handleBookService = () => {
    if (!token) {
      setConfirmationModal({
        text1: "You are not logged in!",
        text2: "Please login to Book Service.",
        btn1Text: "Login",
        btn2Text: "Cancel",
  ...
  return (
    <>
      ...
      <ServiceReviewSlider serviceId={serviceId}/>
      </div>
     
      <Footer />
      {enableWriteReview && <ServiceReviewModal />}
